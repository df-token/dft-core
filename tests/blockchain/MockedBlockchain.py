...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, set_dft_dir


class MockedBlockchain(object):
    MAXNUMBLOCKS = 1000

    def __init__(self, dftnode, time_mock, ntp_mock):
    ...
    def create_block(self, prev_hash, mining_address=None):
        ...
        transactions = []
        block_prev = self.dftnode.get_block_from_hash(prev_hash)
        ...
        dev_config = self.dftnode._chain_manager.get_config_by_block_number(block_new.block_number)
        while not self.dftnode._chain_manager.validate_mining_nonce(blockheader=block_new.blockheader,
    ...
    def validate(self, block):
        if not block.validate(self.dftnode._chain_manager, {}):
    ...
    def add_block(self, block):
        self.validate(block)
        return self.dftnode._chain_manager.add_block(block)

    def add_new_block(self, mining_address=None):
        block_prev = self.dftnode.get_block_last()
    ...
    def create(num_blocks, mining_address=None):
        ...
        with mock.patch('dft.core.misc.ntp.getTime') as ntp_mock, \
                set_dft_dir('no_data'), \
                State() as state, \
            ...
            try:
                ...
                mock_blockchain = MockedBlockchain(dftnode, time_mock, ntp_mock)
