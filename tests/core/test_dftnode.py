...
from dft.core.OptimizedAddressState import OptimizedAddressState
from dft.core.Block import Block
from dft.core.BlockMetadata import BlockMetadata
from dft.core.ESyncState import ESyncState
from dft.core.dftnode import dftNode
from dft.core.State import State
from dft.core.ChainManager import ChainManager
from dft.core.TransactionPool import TransactionPool
from dft.core.txs.CoinBase import CoinBase
from dft.core.txs.TransferTransaction import TransferTransaction
from dft.core.p2p.p2pprotocol import P2PProtocol
from dft.core.p2p.p2pPeerManager import P2PPeerManager
from dft.core.p2p.p2pChainManager import P2PChainManager
from dft.core.node import POW
from dft.generated import dft_pb2
from pydftlib.pydftlib import hstr2bin

from tests.misc.MockHelper.mock_get_tx_metadata import GetTXMetadata
from tests.core.test_State import gen_blocks
from tests.misc.helper import set_dft_dir, get_alice_xmss, get_slave_xmss, replacement_getTime, get_random_xmss
...
class TestdftNodeReal(TestCase):
    @patch('dft.core.misc.ntp.getTime', new=replacement_getTime)
    def setUp(self):
        # You have to set_dft_dir to an empty one, otherwise State will have some Transactions from disk
        with set_dft_dir('no_data'):
            self.db_state = State()
            self.chainmanager = ChainManager(self.db_state)
            self.dftnode = dftNode(mining_address=b'')
            self.dftnode.set_chain_manager(self.chainmanager)

    @patch('dft.core.dftnode.dftNode.block_height', new_callable=PropertyMock, return_value=19)
    def test_get_latest_blocks(self, m_height):
        ...
        latest_blocks = self.dftnode.get_latest_blocks(0, 1)
        ...
        latest_blocks = self.dftnode.get_latest_blocks(0, 2)
        ...
        latest_blocks = self.dftnode.get_latest_blocks(10, 3)
...
class TestdftNode(TestCase):
    ...
    def setUp(self):
        ...
        # As the dftNode is instantiated and torn down for each test, the minuscule or negative diff between present
        # time and start_time can cause problems.
        self.dftnode.start_time -= 10

        self.dftnode._pow = Mock(autospec=POW)
        self.dftnode._pow.miner.lock.__enter__ = Mock()
        self.dftnode._pow.miner.lock.__exit__ = Mock()

        self.dftnode.peer_manager = Mock(autospec=P2PPeerManager, name='mock P2PPeerManager',
                                         known_peer_addresses=set())
        self.dftnode.p2pchain_manager = Mock(autospec=P2PChainManager, name='mock P2PChainManager')
    ...
    @patch('dft.core.BlockMetadata.BlockMetadata.get_block_metadata')
    def test_monitor_chain_state_no_peer_with_higher_difficulty_found(self, mock_get_block_metadata):
        """
        dftNode.monitor_chain_state() basically:
        ...
        self.dftnode.peer_manager.get_better_difficulty.return_value = None
        mock_get_block_metadata.return_value = m_block_metadata
        self.chain_manager.last_block.return_value = m_block

        self.dftnode.monitor_chain_state()

        self.dftnode.peer_manager.monitor_chain_state.assert_called_once()
        self.dftnode.peer_manager.get_better_difficulty.assert_called_once()
    ...
    def test_monitor_chain_state_peer_with_higher_difficulty_found(self, mock_get_block_metadata):
        ...
        self.dftnode.peer_manager.get_better_difficulty.return_value = m_channel
        mock_get_block_metadata.return_value = m_block_metadata
    ...
    def test_start_listening(self):
        ...
        """
        self.assertIsNone(self.dftnode._p2pfactory)
        self.dftnode.start_listening()
        self.assertIsNotNone(self.dftnode._p2pfactory)

    def test_get_address_is_used(self):
        """
        dftNode.get_address_is_used() asks the DB (State) if a particular address has ever been used.
        It also validates the address before sending it to the State.
        """
        self.db_state.get_address_is_used.return_value = True
        result = self.dftnode.get_address_is_used(alice.address)
        ...
        with self.assertRaises(ValueError):
            self.dftnode.get_address_is_used(b'fdsa')

    @patch('dft.core.PaginatedData.PaginatedData.get_paginated_data')
    @patch('dft.core.OptimizedAddressState.OptimizedAddressState.get_optimized_address_state')
    @patch('dft.core.TransactionMetadata.TransactionMetadata.get_tx_metadata')
    def test_get_mini_transactions_by_address(self,
        ...
        """
        dftNode.get_transactions_by_address() returns all the changes in balance caused by a transaction.
        """
        ...
        response = self.dftnode.get_mini_transactions_by_address(alice.address, 3, 1)
    ...
    def test_get_optimized_address_state(self, mock_get_optimized_address_state):
        """
        dftNode.get_address_state() asks the DB (State) for an Address's AddressState, like its nonce, ots index...
        It also validates the address before sending it to the State.
        """
        m_addr_state = Mock(autospec=OptimizedAddressState)
        mock_get_optimized_address_state.return_value = m_addr_state
        result = self.dftnode.get_optimized_address_state(alice.address)
        self.assertEqual(m_addr_state, result)

        # Fetching AddressState for Coinbase Address
        m_addr_state = Mock(autospec=OptimizedAddressState)
        mock_get_optimized_address_state.return_value = m_addr_state
        result = self.dftnode.get_optimized_address_state(config.dev.coinbase_address)
        self.assertEqual(m_addr_state, result)

        with self.assertRaises(ValueError):
            self.dftnode.get_optimized_address_state(b'fdsa')
    ...
    def test_get_addr_from(self):
        ...
        """
        answer = self.dftnode.get_addr_from(slave.pk, alice.address)
        self.assertEqual(answer, alice.address)

        answer = self.dftnode.get_addr_from(slave.pk, None)
    ...
    def test_get_blockidx_from_txhash(self, mock_get_tx_metadata):
        ...
        result = self.dftnode.get_blockidx_from_txhash(b'a txhash')
    ...
    def test_get_block_to_mine(self, m_get_block_metadata):
        ...
        with mock._patch_object(ChainManager, 'last_block') as m_last_block:
            ...
            self.dftnode.get_block_to_mine(alice.address)

            m_last_block.__get__.assert_called_once()
            m_get_block_metadata.assert_called_once()
            self.dftnode._pow.miner.get_block_to_mine.assert_called_once_with(alice.address, self.chain_manager.tx_pool,
                                                                              m_block, 0)

    def test_submit_mined_block(self):
        self.dftnode.submit_mined_block(b'blob')
        self.dftnode._pow.miner.submit_mined_block.assert_called_once_with(b'blob')

    def test_get_node_info(self):
        # I guess this test is useful for making sure every part of dftNode is adequately mocked
        ans = self.dftnode.get_node_info()
        self.assertIsInstance(ans, dft_pb2.NodeInfo)

    @patch('dft.core.LastTransactions.LastTransactions.get_last_txs')
    def test_get_latest_transactions(self, mock_get_last_txs):
        ...
        result = self.dftnode.get_latest_transactions(0, 1)
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].i, 19)

        # Given [0, 1, 2... 19], with offset 2 count 3 should return [15, 16, 17]
        result = self.dftnode.get_latest_transactions(2, 3)
        self.assertEqual(len(result), 3)
    ...
    def test_get_block_timeseries(self):
        ...
        result = self.dftnode.get_block_timeseries(5)
        ...
        result = self.dftnode.get_block_timeseries(3)
        result_converted_from_iterator = [r for r in result]
        self.assertEqual(result_converted_from_iterator, m_blockdps_as_list[2:])

        # If we have a blockheight of 0, return []
        with patch('dft.core.dftnode.dftNode.block_height', new_callable=PropertyMock, return_value=0):
            result = self.dftnode.get_block_timeseries(3)
            result_converted_from_iterator = [r for r in result]  # []
        ...
        result = self.dftnode.get_block_timeseries(6)
        result_converted_from_iterator = [r for r in result]
        self.assertEqual(result_converted_from_iterator, m_blockdps_as_list)

    @patch('dft.core.BlockMetadata.BlockMetadata.get_block_metadata')
    @patch('dft.core.Block.Block.get_block_by_number')
    @patch('dft.core.ChainManager.ChainManager.height', new_callable=PropertyMock, return_value=3)
    def test_get_blockheader_and_metadata(self, m_height, m_get_block_by_number, m_get_block_metadata):
        ...
        result_header, result_metadata = self.dftnode.get_blockheader_and_metadata(0)
        ...
        result_header, result_metadata = self.dftnode.get_blockheader_and_metadata(1)
        self.assertEqual(result_header, blocks[1].blockheader)
        ...
        result_header, result_metadata = self.dftnode.get_blockheader_and_metadata(3)  # get block #3
        ...
        result_header, result_metadata = self.dftnode.get_blockheader_and_metadata(2)
    ...
    def test_submit_send_tx(self):
        ...
        with self.assertRaises(ValueError):
            self.dftnode.submit_send_tx(None)

        self.chain_manager.tx_pool.is_full_pending_transaction_pool.return_value = True
        m_tx = Mock(name='mock Transaction')
        with self.assertRaises(ValueError):
            self.dftnode.submit_send_tx(m_tx)

        self.chain_manager.tx_pool.is_full_pending_transaction_pool.return_value = False
        self.dftnode._p2pfactory = Mock(name='mock P2PFactory')
        self.dftnode.submit_send_tx(m_tx)
        self.dftnode._p2pfactory.add_unprocessed_txn.assert_called_once()


@patch('dft.core.misc.ntp.getTime', new=replacement_getTime)
class TestdftNodeProperties(TestCase):
    @patch('dft.core.misc.ntp.getTime', new=replacement_getTime)
    def setUp(self):
        state = Mock()
        ...
        self.dftnode = dftNode(mining_address=b'')
        self.dftnode.set_chain_manager(self.m_chain_manager)
        self.dftnode.peer_manager = self.m_peer_manager

    def test_state(self):
        # If dftnode._p2pfactory is None, then this should be this value
        self.assertEqual(self.dftnode.state, ESyncState.unknown.value)

        # Else, it should be whatever this part of p2pfactory says
        m_p2pfactory = Mock()
        m_p2pfactory.sync_state.state.value = "test"
        self.dftnode._p2pfactory = m_p2pfactory
        self.assertEqual(self.dftnode.state, "test")

    def test_num_connections(self):
        # If dftnode._p2pfactory is None, then this should return 0
        self.assertEqual(self.dftnode.num_connections, 0)

        # otherwise it should return what p2pfactory's num_connections says
        m_p2pfactory = Mock(num_connections=5)
        self.dftnode._p2pfactory = m_p2pfactory
        self.assertEqual(self.dftnode.num_connections, 5)

    def test_epoch(self):
        self.assertEqual(self.dftnode.epoch, 0)

        self.m_chain_manager._last_block = Mock(block_number=5)
        self.assertEqual(self.dftnode.epoch, (5 // config.dev.blocks_per_epoch))

        self.m_chain_manager._last_block = Mock(block_number=256)
        self.assertEqual(self.dftnode.epoch, (256 // config.dev.blocks_per_epoch))
    ...
    def test_coin_supply(self):
        with mock._patch_object(ChainManager, 'total_coin_supply') as m_total_coin_supply:
            ...
            self.assertEqual(100, self.dftnode.coin_supply)
    ...
    def test_coin_supply_max(self):
        ...
        self.assertEqual(self.dftnode.coin_supply_max, config.dev.max_coin_supply)

    def test_get_peers_stat(self):
        self.dftnode.get_peers_stat()
