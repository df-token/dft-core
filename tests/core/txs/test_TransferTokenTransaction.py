...
class TestTransferTokenTransaction(TestCase):
    ...
    def test_state_validate_tx_custom(self, m_logger):
        ...
        tx = TransferTokenTransaction.create(**params)
        tx.sign(self.alice)
        with patch('dft.core.txs.Transaction.Transaction.fee', new_callable=PropertyMock) as m_fee:
        ...
        tx = TransferTokenTransaction.create(**params)
        tx.sign(self.alice)
        with patch('dft.core.txs.TransferTokenTransaction.TransferTokenTransaction.addrs_to',
        ...
        params = self.default_params()
        params["master_addr"] = b'Bad dft Address'
        with self.assertRaises(ValueError):
            tx = TransferTokenTransaction.create(**params)

        params = self.default_params()
        params["addrs_to"] = [self.bob.address, b'Bad dft address']
    ...
    def test_validate_extended(self, m_validate_slave, m_logger):
        ...
        tokens.data[(self.alice.address, tx.token_txhash)] = dft_pb2.TokenBalance(balance=1000,
                                                                                  decimals=0,
                                                                                  delete=False)
        ...
        state_container.tokens.data[(self.alice.address, tx.token_txhash)] = dft_pb2.TokenBalance(balance=1000,
                                                                                                  decimals=0,
                                                                                                  delete=False)
        ...
        # fee = -1
        with patch('dft.core.txs.TransferTokenTransaction.TransferTokenTransaction.fee',
        ...
        # total_amount = -1
        with patch('dft.core.txs.TransferTokenTransaction.TransferTokenTransaction.total_amount',
        ...
        tokens = Indexer(b'token', None)
        tokens.data[(self.alice.address, tx.token_txhash)] = dft_pb2.TokenBalance(balance=99,
                                                                                  decimals=0,
                                                                                  delete=False)
        state_container.tokens = tokens
        result = tx._validate_extended(state_container)
        ...
        tx = TransferTokenTransaction.create(**params)
        tx.sign(self.alice)
        with patch('dft.core.config', autospec=True) as m_config:
