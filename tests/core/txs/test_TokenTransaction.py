...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, get_slave_xmss, set_dft_dir

logger.initialize_default()


@patch('dft.core.txs.Transaction.logger')
class TestTokenTransaction(TestCase):
    ...
    def setUp(self):
        self.initial_balances_valid = [dft_pb2.AddressAmount(address=self.alice.address, amount=1000),
                                       dft_pb2.AddressAmount(address=self.bob.address, amount=1000)]

        self.params = {"symbol": b'dft',
    ...
    def test_create(self, m_logger):
        ...
        initial_balances.append(dft_pb2.AddressAmount(address=self.alice.address,
                                                      amount=400000000))
        initial_balances.append(dft_pb2.AddressAmount(address=self.bob.address,
                                                      amount=200000000))
    ...
    def test_create_negative_fee(self, m_logger):
        with self.assertRaises(ValueError):
            TokenTransaction.create(symbol=b'dft',
                                    name=b'Quantum Resistant Ledger',
                                    owner=self.alice.address,
                                    decimals=4,
                                    initial_balances=[],
    ...
    def test_to_json(self, m_logger):
        ...
        initial_balances.append(dft_pb2.AddressAmount(address=self.bob.address,
                                                      amount=200000000))
        tx = TokenTransaction.create(symbol=b'dft',
                                     name=b'Quantum Resistant Ledger',
    ...
    def test_from_json(self, m_logger):
        ...
        self.assertEqual('01030038ea6375069f8272cc1a6601b3c76c21519455603d370036b97c779ada356'
                         ...
                         bin2hstr(tx.PK))
        self.assertEqual(b'dft', tx.symbol)
    ...
    @patch('dft.core.txs.Transaction.Transaction.validate_slave', return_value=True)
    def test_validate_extended(self, m_validate_slave, m_logger):
        ...
        with patch('dft.core.txs.TokenTransaction.TokenTransaction.addr_from',
                   new_callable=PropertyMock) as m_addr_from:
            m_addr_from.return_value = b'Invalid Address'
            result = tx._validate_extended(state_container)
            self.assertFalse(result)

        with patch('dft.core.txs.TokenTransaction.TokenTransaction.owner', new_callable=PropertyMock) as m_owner:
            m_owner.return_value = b'Invalid Address'
            result = tx._validate_extended(state_container)
            self.assertFalse(result)

        with patch('dft.core.txs.TokenTransaction.TokenTransaction.initial_balances',
                   new_callable=PropertyMock) as m_address_balance:
            m_address_balance.return_value = [dft_pb2.AddressAmount(address=b'Invalid Address 1', amount=1000),
                                              dft_pb2.AddressAmount(address=b'Invalid Address 2', amount=1000)]
            result = tx._validate_extended(state_container)
            self.assertFalse(result)
        ...
        tx = self.make_tx(symbol=b'dftSdftSdft')
        tx.sign(self.alice)
        ...
        with self.assertRaises(ValueError):
            initial_balances_0_0 = [dft_pb2.AddressAmount(address=self.alice.address, amount=0),
                                    dft_pb2.AddressAmount(address=self.bob.address, amount=0)]
            tx = self.make_tx(initial_balances=initial_balances_0_0)
            tx.sign(self.alice)
            self.assertFalse(tx._validate_extended(state_container))

        # Fee is -1
        with patch('dft.core.txs.TokenTransaction.TokenTransaction.fee', new_callable=PropertyMock) as m_fee:
        ...
        # tx = self.make_tx()
        # tx.sign(self.alice)
        # with patch('dft.core.txs.TokenTransaction.TokenTransaction.initial_balances', new_callable=PropertyMock) as m_i_balances:
    ...
    def test_affected_address(self, m_logger):
        ...
        result = set()
        self.initial_balances_valid.append(dft_pb2.AddressAmount(address=slave.address, amount=1000))
        tx = TokenTransaction.create(symbol=b'dft',
                                     name=b'Quantum Resistant Ledger',
                                     owner=self.alice.address,
                                     decimals=15,
