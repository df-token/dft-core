...
from tests.misc.helper import replacement_getTime, set_dft_dir, get_alice_xmss, get_bob_xmss
...
class TestTransactionPool(TestCase):
    ...
    @patch('dft.core.TransactionPool.TransactionPool.is_full_transaction_pool', autospec=True)
    ...
    @patch('dft.core.TransactionPool.config', autospec=True)
    ...
    def test_get_tx_index_from_pool(self):
        ...
        tx3 = make_tx(txhash=b'dftpink')

        self.txpool.add_tx_to_pool(tx1, 1, replacement_getTime())
        self.txpool.add_tx_to_pool(tx2, 1, replacement_getTime())
        self.txpool.add_tx_to_pool(tx3, 1, replacement_getTime())

        idx = self.txpool.get_tx_index_from_pool(b'dftpink')
    ...
    @patch('dft.core.TransactionPool.TransactionPool.is_full_pending_transaction_pool', autospec=True)
    ...
    @patch('dft.core.TransactionPool.logger')
    @patch('dft.core.TransactionPool.TransactionPool.is_full_pending_transaction_pool', autospec=True)
    ...
    @patch('dft.core.TransactionPool.logger')
    @patch('dft.core.txs.Transaction.Transaction.from_pbdata', return_value=make_tx())
    @patch('dft.core.TransactionPool.TransactionPool.add_tx_to_pool', return_value=True)
    ...
    @patch('dft.core.txs.Transaction.Transaction.from_pbdata', new=replacement_from_pbdata)
    ...
    @patch('dft.core.TransactionInfo.config', autospec=True)
    @patch('dft.core.TransactionPool.TransactionPool.is_full_transaction_pool', return_value=False)
