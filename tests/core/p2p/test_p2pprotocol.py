...
from dft.core.p2p import p2pPeerManager
from dft.core.p2p.p2pfactory import P2PFactory, p2p_msg_priority
...
class TestP2PProtocol(TestCase):
    ...
    def setUp(self):
        with patch('dft.core.misc.ntp.getTime', new=replacement_getTime):
        ...
        self.channel.factory._dft_node = Mock(autospec=dftNode)
        self.channel.factory._dft_node.peer_manager = Mock(autospec=p2pPeerManager)
        self.channel.factory._dft_node.peer_manager.is_banned = MagicMock(return_value=False)
        self.channel.factory._dft_node.peer_manager.trusted_addresses = MagicMock(return_value=[])
    ...
    @patch('dft.core.misc.ntp.getTime')
    @patch('dft.core.p2p.p2pprotocol.logger', autospec=True)
    @patch('dft.core.p2p.p2pprotocol.config.dev', autospec=True)
    def test_dataReceived_too_big(self, config_dev, logger, getTime):
        ...
        config_dev.trust_min_msdftount = 10
        getTime.return_value = 1525078652.9991353
