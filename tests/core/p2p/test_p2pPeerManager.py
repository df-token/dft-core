...
from tests.misc.helper import set_dft_dir
...
def make_node_chain_state():
    node_chain_state = dft_pb2.NodeChainState(block_number=0,
                                              header_hash=b'',
...
class TestP2PPeerManager(TestCase):
    ...
    def test_load_peer_addresses_peer_dft(self):
        """
        Should load peers from peers.dft AND from config.user.peer_list
        """
        with set_dft_dir('peers') as tmp_dir:
            self.peer_manager.peers_path = os.path.join(tmp_dir, 'peers.json')

            self.peer_manager.load_peer_addresses()

            # This was in peers.dft
    ...
    def test_load_peer_addresses_no_file(self):
        """
        If no peers.dft exists, use config.user.peer_list
        """
        with set_dft_dir('no_data') as tmp_dir:
    ...
    @patch('dft.core.p2p.p2pPeerManager.logger', autospec=True)
    def test_load_peer_addresses_corrupt_file(self, logger):
        """
        If peers.dft is corrupt, load_peer_addresses should rewrite it with valid peers from config.user.peer_list
        """
        with set_dft_dir('peers') as tmp_dir:
    ...
    def test_update_peer_addresses_also_connects_to_new_peers(self, logger):
        ...
        with set_dft_dir('no_data') as tempdir:
    ...
    def test_handle_version(self):
        ...
        message = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.VE,
    ...
    def test_handle_version_empty_version_message(self):
        ...
        message = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.VE,
                                              veData=dftlegacy_pb2.VEData(version='',
                                                                          genesis_prev_hash=config.user.genesis_prev_headerhash,
                                                                          rate_limit=config.user.peer_rate_limit))
        self.peer_manager.handle_version(channel, message)
    ...
    def test_handle_peer_list_works(self, logger):
        ...
        """
        peer_list_message = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.PL,
                                                        plData=dftlegacy_pb2.PLData(
    ...
    def test_handle_peer_list_empty_peer_list_message(self, logger):
        ...
        """
        peer_list_message = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.PL,
                                                        plData=dftlegacy_pb2.PLData(peer_ips={}, public_port=9000))
        channel = make_channel()
    ...
    @patch('dft.core.p2p.p2pPeerManager.logger', autospec=True)
    @patch('dft.core.p2p.p2pPeerManager.config', autospec=True)
    ...
    def test_handle_sync(self):
        ...
        """
        sync_message = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.SYNC,
                                                   syncData=dftlegacy_pb2.SYNCData(state='Synced'))
        channel = make_channel()
    ...
    def test_handle_sync_unsynced(self):
        ...
        """
        sync_message = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.SYNC,
                                                   syncData=dftlegacy_pb2.SYNCData(state='Unsynced'))
        channel = make_channel()
    ...
    def test_handle_sync_blank(self):
        ...
        """
        sync_message = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.SYNC,
                                                   syncData=dftlegacy_pb2.SYNCData(state=''))
        channel = make_channel()
    ...
    def test_handle_chain_state_works(self):
        ...
        chain_state_message = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.CHAINSTATE,
    ...
    def test_handle_p2p_acknowledgement(self):
        ...
        """
        ack = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.P2P_ACK,
                                          p2pAckData=dft_pb2.P2PAcknowledgement(bytes_processed=15))
        channel = make_channel()
    ...
    def test_handle_p2p_acknowledgement_negative_bytes_processed(self):
        ...
        """
        ack = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.P2P_ACK,
                                          p2pAckData=dft_pb2.P2PAcknowledgement(bytes_processed=20))
        channel = make_channel()
    ...
    @patch('dft.core.p2p.p2pprotocol.P2PProtocol.peer')
    @patch('dft.core.p2p.p2pprotocol.P2PProtocol.send')
    def test_trusted_message_count(self, send, get_peer):
        channel = P2PProtocol()
        with mock.patch('dft.core.misc.ntp.getTime') as time_mock:
            ...
            for _ in range(config.dev.trust_min_msdftount - 1):
    ...
    def test_trusted_time(self, send, get_peer):
        ...
        with mock.patch('dft.core.misc.ntp.getTime') as time_mock:
            ...
            for _ in range(config.dev.trust_min_msdftount):
                buffer = bytes(hstr2bin('000000191a170a0776657273696f6e120c67656e657369735f68617368'))
                channel.dataReceived(buffer)
