...
@patch('dft.core.p2p.p2pChainManager.logger')
class TestP2PChainManager(TestCase):
    ...
    def test_handle_fetch_block(self, m_logger):
        ...
        self.channel.factory.get_block_by_number.return_value = Mock(autospec=Block, pbdata=dft_pb2.Block())
        msg = make_message(func_name=dftlegacy_pb2.LegacyMessage.FB, fbData=dftlegacy_pb2.FBData(index=1))
    ...
    def test_handle_fetch_block_bad_height(self, m_logger):
        ...
        """
        self.channel.factory.chain_height = 10
        msg = make_message(func_name=dftlegacy_pb2.LegacyMessage.FB, fbData=dftlegacy_pb2.FBData(index=100))
        self.manager.handle_fetch_block(self.channel, msg)
    ...
    def test_handle_push_block(self, m_logger):
        ...
        :return:
        """
        msg = make_message(func_name=dftlegacy_pb2.LegacyMessage.PB, pbData=dftlegacy_pb2.PBData(block=dft_pb2.Block()))
        self.manager.handle_push_block(self.channel, msg)

        self.channel.factory.block_received.assert_called()

    @patch('dft.core.p2p.p2pChainManager.Block')
    def test_handle_push_block_bad_block(self, m_Block, m_logger):
        ...
        msg = make_message(func_name=dftlegacy_pb2.LegacyMessage.PB)
        self.manager.handle_push_block(self.channel, msg)
    ...
    def test_handle_block(self, m_logger):
        ...
        :return:
        """
        msg = make_message(func_name=dftlegacy_pb2.LegacyMessage.BK,
                           block=dft_pb2.Block(),
                           mrData=dftlegacy_pb2.MRData()
                           )
    ...
    def test_handle_block_height_incoming_request(self, m_logger):
        ...
        incoming_request = make_message(func_name=dftlegacy_pb2.LegacyMessage.BH,
    ...
    def test_handle_block_height_incoming_request_but_node_at_blockheight_zero(self, m_logger):
        ...
        incoming_request = make_message(func_name=dftlegacy_pb2.LegacyMessage.BH,
                                        bhData=dft_pb2.BlockHeightData(
                                            block_number=0,
                                        ))
        self.manager.handle_block_height(self.channel, incoming_request)
    ...
    def test_handle_block_height_incoming_information(self, m_logger):
        ...
        incoming_info = make_message(func_name=dftlegacy_pb2.LegacyMessage.BH,
                                     bhData=dft_pb2.BlockHeightData(
                                         block_number=1,
    ...
    def test_handle_node_headerhash_peer_has_no_headerhashes(self, m_logger):
        ...
        """
        self.channel.factory.get_headerhashes.return_value = dft_pb2.NodeHeaderHash()
        incoming_request = make_message(func_name=dftlegacy_pb2.LegacyMessage.HEADERHASHES,
                                        nodeHeaderHash=dft_pb2.NodeHeaderHash(block_number=0)
                                        )
    ...
    def test_handle_node_headerhash_peer_has_headerhashes(self, m_logger):
        ...
        """
        incoming_info = make_message(func_name=dftlegacy_pb2.LegacyMessage.HEADERHASHES,
                                     nodeHeaderHash=dft_pb2.NodeHeaderHash(block_number=3,
