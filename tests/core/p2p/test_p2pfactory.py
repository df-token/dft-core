...
from dft.core.Message import Message
from dft.core.MessageRequest import MessageRequest
...
from dft.generated import dft_pb2, dftlegacy_pb2
...
def make_message(**kwargs):
    return dftlegacy_pb2.LegacyMessage(**kwargs)
...
@patch('dft.core.p2p.p2pfactory.logger', autospec=logger, name="Mock Logger")
@patch('dft.core.p2p.p2pfactory.reactor', autospec=reactor, name="My Mock Reactor")
class TestP2PFactory(TestCase):
    def __init__(self, *args, **kwargs):
        super(TestP2PFactory, self).__init__(*args, **kwargs)

    def setUp(self):
        self.m_dftnode = Mock(autospec=dftNode, name='Fake dftNode')
        self.m_dftnode.peer_manager = Mock(autospec=P2PPeerManager, name='Fake PeerManager')
        self.m_dftnode.peer_manager.is_banned.return_value = False
        self.m_dftnode.peer_manager.load_known_peers.return_value = ['8.8.8.8', '9.9.9.9']
        ...
        self.factory = P2PFactory(chain_manager=ChainManager(state=Mock(autospec=State)), sync_state=None, dft_node=self.m_dftnode)
    ...
    def test_create_factory(self, m_reactor, m_logger):
        factory = P2PFactory(chain_manager=ChainManager(None), sync_state=None, dft_node=None)
        self.assertEqual(P2PProtocol, factory.protocol,
                         "Factory has not been assigned the expected protocol")

    @patch('dft.core.p2p.p2pfactory.config', autospec=True)
    ...
    def test_add_connection_wont_connect_to_itself(self, m_reactor, m_logger):
        ...
        self.factory._dft_node.peer_manager.known_peer_addresses = [make_address('1.1.1.1'),
        ...
        self.assertEqual(self.factory.num_connections, 3)
        self.factory._dft_node.peer_manager.extend_known_peers.assert_called_once_with(
    ...
    def test_monitor_connections(self, m_reactor, m_logger):
        ...
        with patch('dft.core.p2p.p2pfactory.config', autospec=True) as m_config:
    ...
    def test_request_full_message(self, m_reactor, m_logger):
        ...
        mrData = dftlegacy_pb2.MRData(type=dftlegacy_pb2.LegacyMessage.SL, hash=b'1234')
    ...
    def test_broadcast(self, m_reactor, m_logger):
        # broadcast msg_type is dftlegacy_pb2.LegacyMessage
        self.factory.broadcast(dftlegacy_pb2.LegacyMessage.TX, b'1234')
        self.channel_1.send.assert_called_once()
        self.channel_2.send.assert_called_once()
    ...
    def test_broadcast_does_not_broadcast_to_peers_known_to_have_the_mr(self, m_reactor, m_logger):
        ...
        self.factory.master_mr.requested_hash[b'1234'] = message_request
        self.factory.broadcast(dftlegacy_pb2.LegacyMessage.SL, b'1234')
        self.channel_1.send.assert_called_once()
...
class TestP2PFactoryCompareAndSync(TestCase):
    def setUp(self):
        ...
        self.node_header_hash = dft_pb2.NodeHeaderHash(
            block_number=1,
...
class TestP2PFactoryPeerFetchBlock(TestCase):
    def setUp(self):
        ...
        self.m_dftnode.is_banned.return_value = False
        self.factory.add_connection(self.channel_1)
        self.factory.add_connection(self.channel_2)
        self.factory.add_connection(self.channel_3)
        ...
        self.factory._target_node_header_hash = dft_pb2.NodeHeaderHash(
            block_number=1,
    ...
    def test_peer_fetch_block_block_not_found_and_retry(self, m_reactor, m_logger):
        ...
        self.m_dftnode.peer_manager.ban_channel.assert_called_once_with(self.channel_1)
...
class TestP2PFactoryBlockReceived(TestCase):
    def setUp(self):
        ...
        self.factory = P2PFactory(chain_manager=Mock(autospec=ChainManager), sync_state=None, dft_node=self.m_dftnode)
        self.factory.pow = Mock(autospec=POW)
