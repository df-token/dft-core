...
from pydftlib.pydftlib import str2bin, hstr2bin, bin2hstr
...
from dft.services.PublicAPIService import PublicAPIService
from tests.blockchain.MockedBlockchain import MockedBlockchain
from tests.misc.helper import get_alice_xmss, get_bob_xmss, replacement_getTime, set_dft_dir
...
class TestPublicAPI(TestCase):
    ...
    def test_getNodeState(self):
        ...
        dftnode.set_chain_manager(chain_manager)
        dftnode._p2pfactory = p2p_factory
        dftnode._pow = p2p_factory.pow

        service = PublicAPIService(dftnode)
        node_state = service.GetNodeState(request=dft_pb2.GetNodeStateReq, context=None)

        # self.assertEqual(__version__, node_state.info.version)  # FIXME
        self.assertEqual(dft_pb2.NodeInfo.UNSYNCED, node_state.info.state)
    ...
    def test_getKnownPeers(self):
        ...
        dftnode._p2pfactory = p2p_factory
        dftnode._pow = p2p_factory.pow
        dftnode.peer_manager = Mock()
        dftnode.peer_manager.known_peer_addresses = ['127.0.0.1', '192.168.1.1']

        service = PublicAPIService(dftnode)
        response = service.GetKnownPeers(request=dft_pb2.GetKnownPeersReq, context=None)
    ...
    def test_getStats_timeseries(self):
        with MockedBlockchain.create(10) as mock_blockchain:
            service = PublicAPIService(mock_blockchain.dftnode)
            request = dft_pb2.GetStatsReq(include_timeseries=1)
            stats = service.GetStats(request=request, context=None)
    ...
    def test_getAddressState(self):
        with set_dft_dir('no_data'):
            ...
            dftnode.set_chain_manager(chain_manager)
            dftnode._p2pfactory = p2p_factory
            dftnode._peer_addresses = ['127.0.0.1', '192.168.1.1']
    ...
    def test_getObject(self):
        ...
        with set_dft_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           block.block_number,
                                           dft_pb2.BlockNumberMapping(headerhash=block.headerhash),
                                           None)
            context = Mock(spec=ServicerContext)
            request = dft_pb2.GetObjectReq()
    ...
    def test_getLatestData(self):
        with set_dft_dir('no_data'):
            ...
            dftnode = dftNode(mining_address=b'')
            dftnode.set_chain_manager(chain_manager)
            dftnode.get_block_from_index = MagicMock(return_value=None)
            ...
            request = dft_pb2.GetLatestDataReq(filter=dft_pb2.GetLatestDataReq.ALL,
                                               offset=0,
                                               quantity=3)
    ...
    def test_getAddressFromPK(self):
        ...
        request = dft_pb2.GetAddressFromPKReq(pk=bytes(pk))
    ...
    @patch('dft.core.config.DevConfig', new_callable=PropertyMock)
    def test_getMiniTransactionsByAddress(self, mock_dev_config):
        ...
        with set_dft_dir('no_data'):
            ...
            TransactionMetadata.update_tx_metadata(db_state, block, None)
            request = dft_pb2.GetMiniTransactionsByAddressReq(address=alice_xmss.address,
                                                              item_per_page=10,
                                                              page_number=1)
    ...
    def test_getTransaction(self):
        ...
        context = Mock(spec=ServicerContext)
        request = dft_pb2.GetTransactionReq()
    ...
    def test_getTotalBalance(self):
        with set_dft_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = dft_pb2.GetTotalBalanceReq()
    ...
    def test_getOTS(self):
        with set_dft_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = dft_pb2.GetOTSReq(address=alice_xmss.address)
    ...
    def test_getBlock(self):
        with set_dft_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = dft_pb2.GetBlockReq(header_hash=b.headerhash)
    ...
    def test_getBlockByNumber(self):
        with set_dft_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           b.block_number,
                                           dft_pb2.BlockNumberMapping(headerhash=b.headerhash),
                                           None)
            db_state.get_block_by_number = MagicMock(return_value=b)

            context = Mock(spec=ServicerContext)
            request = dft_pb2.GetBlockByNumberReq(block_number=b.block_number)
