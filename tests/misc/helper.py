...
from pydftlib.pydftlib import XmssFast
...
@contextlib.contextmanager
def set_dft_dir(data_name):
    dst_dir = tempfile.mkdtemp()
    prev_val = config.user.dft_dir
    try:
        test_path = os.path.dirname(os.path.abspath(__file__))
        ...
        config.user.dft_dir = dst_dir
        yield dst_dir
    finally:
        shutil.rmtree(dst_dir)
        config.user.dft_dir = prev_val
...
def mocked_genesis():
    ...
    with mock.patch('dft.core.GenesisBlock.GenesisBlock.instance'):
...
def get_token_transaction(xmss1, xmss2, amount1=400000000, amount2=200000000, fee=1) -> TokenTransaction:
    ...
    initial_balances.append(dft_pb2.AddressAmount(address=xmss1.address,
                                                  amount=amount1))
    initial_balances.append(dft_pb2.AddressAmount(address=xmss2.address,
                                                  amount=amount2))

    return TokenTransaction.create(symbol=b'dft',
...
def gen_blocks(block_count, state, miner_address):
    ...
    with mock.patch('dft.core.misc.ntp.getTime') as time_mock:
        ...
        for i in range(0, block_count):
            ...
            bm = dft_pb2.BlockNumberMapping(headerhash=block.headerhash,
