...
from dft.core.misc import ntp, logger, set_logger
from dft.core.dftnode import dftNode
from dft.services.services import start_services
...
def parse_arguments():
    ...
    parser.add_argument('--miningAddress', dest='mining_address', required=False,
                        help="dft Wallet address on which mining reward has to be credited.")
...
def main():
    ...
    logger.debug("=====================================================================================")
    logger.info("dft Path: %s", args.dft_dir)
    ...
    dftnode = dftNode(mining_address=mining_address)
    dftnode.set_chain_manager(chain_manager)

    set_logger.set_logger(args, dftnode.sync_state)

    #######
    # NOTE: Keep assigned to a variable or might get collected
    admin_service, grpc_service, mining_service, debug_service = start_services(dftnode)

    dftnode.start_listening()

    dftnode.start_pow(args.mining_thread_count)

    logger.info('dft blockchain ledger %s', config.dev.version)
    if config.user.mining_enabled:
