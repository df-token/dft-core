...
from pydftlib.pydftlib import hstr2bin, mnemonic2bin, bin2hstr, dftHelper

from dft.core import config
from dft.core.AddressState import AddressState
from dft.daemon.helper import logger
from dft.daemon.helper.DaemonHelper import WalletDecryptionError, Wallet, UNRESERVED_OTS_INDEX_START
from dft.services.WalletAPIService import WalletAPIService
from dft.generated import dft_pb2, dft_pb2_grpc, dftwallet_pb2
from dft.generated.dftwallet_pb2_grpc import add_WalletAPIServicer_to_server
...
from dft.crypto.xmss import XMSS

CONNECTION_TIMEOUT = 30
config.create_path(config.user.wallet_dir)
pid = os.path.join(config.user.wallet_dir, 'dft_walletd.pid')


class WalletD:
    def __init__(self):
        self._wallet_path = os.path.join(config.user.wallet_dir, 'walletd.json')
        self._public_stub = dft_pb2_grpc.PublicAPIStub(grpc.insecure_channel(config.user.public_api_server))
        self._wallet = None
        self._passphrase = None
        self.load_wallet()

    def to_plain_blocks(self, block):
        pheader = dftwallet_pb2.PlainBlockHeader()
        ...
        pblock = dftwallet_pb2.PlainBlock()
        ...
        for genesis_balance in block.genesis_balance:
            pgb = dftwallet_pb2.PlainGenesisBalance()
    ...
    def get_address_state(self, qaddress: str) -> AddressState:
        request = dft_pb2.GetAddressStateReq(address=bytes(hstr2bin(qaddress[1:])))
    ...
    def sign_and_push_transaction(self,
        ...
        push_transaction_req = dft_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
        push_transaction_resp = self._public_stub.PushTransaction(push_transaction_req, timeout=CONNECTION_TIMEOUT)
        if push_transaction_resp.error_code != dft_pb2.PushTransactionResp.SUBMITTED:
    ...
    def is_slave(self, master_address: bytes, slave_pk: bytes) -> bool:
        request = dft_pb2.IsSlaveReq(master_address=master_address,
                                     slave_pk=slave_pk)
    ...
    def get_unused_ots_index(self, address: bytes, unused_ots_index_from: int) -> Optional[int]:
        request = dft_pb2.GetOTSReq(address=address,
    ...
    def relay_token_txn(self,
        ...
        for idx, qaddress in enumerate(qaddresses):
            initial_balances.append(dft_pb2.AddressAmount(address=self.qaddress_to_address(qaddress),
                                                          amount=amounts[idx]))
    ...
    def get_mini_transactions_by_address(self, qaddress: str, item_per_page: int, page_number: int) -> tuple:
        ...
        response = self._public_stub.GetMiniTransactionsByAddress(
            dft_pb2.GetMiniTransactionsByAddressReq(address=address,
    ...
    def get_transaction(self, tx_hash: str):
        ...
        response = self._public_stub.GetTransaction(dft_pb2.GetTransactionReq(tx_hash=txhash))
    ...
    def get_balance(self, qaddress: str) -> int:
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetBalance(dft_pb2.GetBalanceReq(address=address))
    ...
    def get_total_balance(self) -> int:
        ...
        response = self._public_stub.GetTotalBalance(dft_pb2.GetTotalBalanceReq(addresses=addresses))
        return response.balance

    def get_ots(self, qaddress: str):
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetOTS(dft_pb2.GetOTSReq(address=address))
        return response.ots_bitfield_by_page, response.next_unused_ots_index, response.unused_ots_index_found

    def get_height(self) -> int:
        response = self._public_stub.GetHeight(dft_pb2.GetHeightReq())
        return response.height

    def get_block(self, header_hash: str):
        headerhash = bytes(hstr2bin(header_hash))
        response = self._public_stub.GetBlock(dft_pb2.GetBlockReq(header_hash=headerhash))
        return self.to_plain_blocks(response.block)

    def get_block_by_number(self, block_number: int):
        response = self._public_stub.GetBlockByNumber(dft_pb2.GetBlockByNumberReq(block_number=block_number))
        return self.to_plain_blocks(response.block)

    def get_address_from_pk(self, pk: str) -> str:
        return self.address_to_qaddress(dftHelper.getAddress(bytes(hstr2bin(pk))))

    def get_node_info(self):
        return self._public_stub.GetNodeState(dft_pb2.GetNodeStateReq())
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='dft node')
    parser.add_argument('--dftdir', '-d', dest='dft_dir', default=config.user.dft_dir,
    ...
    parser.add_argument('--network-type', dest='network_type', choices=['mainnet', 'testnet'],
                        default='mainnet', required=False, help="Runs dft Testnet Node")
...
def main():
    ...
    config.user.dft_dir = os.path.expanduser(os.path.normpath(args.dft_dir) + dft_dir_post_fix)
    config.create_path(config.user.dft_dir, copy_files)
    config.user.load_yaml(config.user.config_path)

    daemon = Daemonize(app="dft_walletd", pid=pid, action=run)
    daemon.start()
