...
from pydftlib.pydftlib import bin2hstr  # noqa
from twisted.internet.protocol import Protocol, connectionDone

from dft.core import config
from dft.core.OutgoingMessage import OutgoingMessage
from dft.core.misc import logger, ntp
from dft.core.p2p.IPMetadata import IPMetadata
from dft.core.p2p.p2pObservable import P2PObservable
...
class P2PProtocol(Protocol):
    ...
    def peer_manager(self):
        ...
        return self.factory._dft_node.peer_manager

    @property
    def p2pchain_manager(self):
        # FIXME: this is breaking encapsulation
        return self.factory._dft_node.p2pchain_manager

    @property
    def tx_manager(self):
        # FIXME: this is breaking encapsulation
        return self.factory._dft_node.tx_manager
    ...
    def dataReceived(self, data: bytes) -> None:
        ...
        for msg in self._parse_buffer(read_bytes):
            ...
            if self._valid_message_count < config.dev.trust_min_msdftount * 2:
                # Avoid overflows
                self._valid_message_count += 1

            self._observable.notify(msg)

        if msg is not None and read_bytes[0] and msg.func_name != dftlegacy_pb2.LegacyMessage.P2P_ACK:
            p2p_ack = dft_pb2.P2PAcknowledgement(bytes_processed=read_bytes[0])
            msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.P2P_ACK,
                                              p2pAckData=p2p_ack)
            self.send(msg)
    ...
    def send(self, message: dftlegacy_pb2.LegacyMessage):
    ...
    def _wrap_message(protobuf_obj) -> Optional[bytes]:
        ...
        :rtype: bytes
        >>> veData = dftlegacy_pb2.VEData(version="version", genesis_prev_hash=b'genesis_hash')
        >>> msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.VE, veData=veData)
        >>> bin2hstr(P2PProtocol._wrap_message(msg))
        '000000191a170a0776657273696f6e120c67656e657369735f68617368'

                msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.PL,
                                          plData=dftlegacy_pb2.PLData(peer_ips=trusted_peers,
                                                                      public_port=config.user.p2p_public_port))

        >>> plData = dftlegacy_pb2.PLData(peer_ips=[])
        >>> msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.PL, plData=plData)
        >>> bin2hstr(P2PProtocol._wrap_message(msg))
        '0000000408012200'
        """
    ...
    def _parse_buffer(self, total_read):
        ...
        while self._buffer:
            ...
            try:
                ...
                message = dftlegacy_pb2.LegacyMessage()
    ...
    def send_version_request(self):
        msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.VE,
                                          veData=dftlegacy_pb2.VEData())
        self.send(msg)
    ...
    def send_sync(self, synced=False):
        ...
        msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.SYNC,
                                          syncData=dftlegacy_pb2.SYNCData(state=state_str))
        self.send(msg)
    ...
    def send_fetch_block(self, block_idx):
        ...
        msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.FB,
                                          fbData=dftlegacy_pb2.FBData(index=block_idx))
        self.send(msg)
    ...
    def send_get_headerhash_list(self, current_block_height):
        ...
        node_header_hash = dft_pb2.NodeHeaderHash(block_number=start_blocknumber,
                                                  headerhashes=[])
        msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.HEADERHASHES,
