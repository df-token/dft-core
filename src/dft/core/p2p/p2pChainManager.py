...
class P2PChainManager(P2PBaseObserver):
    ...
    def new_channel(self, channel):
        channel.register(dftlegacy_pb2.LegacyMessage.BK, self.handle_block)
        channel.register(dftlegacy_pb2.LegacyMessage.FB, self.handle_fetch_block)
        channel.register(dftlegacy_pb2.LegacyMessage.PB, self.handle_push_block)
        channel.register(dftlegacy_pb2.LegacyMessage.BH, self.handle_block_height)
        channel.register(dftlegacy_pb2.LegacyMessage.HEADERHASHES, self.handle_node_headerhash)

    def handle_fetch_block(self, source, message: dftlegacy_pb2.LegacyMessage):  # Fetch Request for block
        """
        Fetch Block
        Sends the request for the block.
        :return:
        """
        P2PBaseObserver._validate_message(message, dftlegacy_pb2.LegacyMessage.FB)
        ...
        if 0 < block_number <= source.factory.chain_height:
            ...
            msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.PB,
                                              pbData=dftlegacy_pb2.PBData(block=block.pbdata))
            source.send(msg)

    def handle_push_block(self, source, message: dftlegacy_pb2.LegacyMessage):
        """
        Push Block
        ...
        """
        # FIXME: Later rename
        P2PBaseObserver._validate_message(message, dftlegacy_pb2.LegacyMessage.PB)
    ...
    def handle_block(self, source, message: dftlegacy_pb2.LegacyMessage):  # block received
        """
        Block
        This function processes any new block received.
        :return:
        """
        P2PBaseObserver._validate_message(message, dftlegacy_pb2.LegacyMessage.BK)
        try:
        ...
        source.factory.master_mr.register(dftlegacy_pb2.LegacyMessage.BK, block.headerhash, message.block)

    def handle_block_height(self, source, message: dftlegacy_pb2.LegacyMessage):
        """
        ...
        if message.bhData.block_number == 0:
            ...
            if block.block_number == 0:
                return
            bhdata = dft_pb2.BlockHeightData(block_number=block.block_number,
    ...
    def handle_node_headerhash(self, source, message: dftlegacy_pb2.LegacyMessage):
        """
