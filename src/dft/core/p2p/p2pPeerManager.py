...
from dft.core.misc.expiring_set import ExpiringSet
from dft.core.notification.Observable import Observable
from dft.core.notification.ObservableEvent import ObservableEvent
...
class P2PPeerManager(P2PBaseObserver):
    ...
    def trusted_peer(self, channel: P2PProtocol):
        ...
        if channel.valid_message_count < config.dev.trust_min_msdftount:
            return False
    ...
    def new_channel(self, channel):
        ...
        self._peer_node_status[channel] = dft_pb2.NodeChainState(block_number=0,
                                                                 header_hash=b'',
                                                                 cumulative_difficulty=b'\x00' * 32,
                                                                 timestamp=ntp.getTime())
        channel.register(dftlegacy_pb2.LegacyMessage.VE, self.handle_version)
        channel.register(dftlegacy_pb2.LegacyMessage.PL, self.handle_peer_list)
        channel.register(dftlegacy_pb2.LegacyMessage.CHAINSTATE, self.handle_chain_state)
        channel.register(dftlegacy_pb2.LegacyMessage.SYNC, self.handle_sync)
        channel.register(dftlegacy_pb2.LegacyMessage.P2P_ACK, self.handle_p2p_acknowledgement)
    ...
    def handle_version(self, source, message: dftlegacy_pb2.LegacyMessage):
        """
        Version
        ...
        """
        self._validate_message(message, dftlegacy_pb2.LegacyMessage.VE)

        if not message.veData.version:
            msg = dftlegacy_pb2.LegacyMessage(
                func_name=dftlegacy_pb2.LegacyMessage.VE,
                veData=dftlegacy_pb2.VEData(version=config.dev.version,
    ...
    def handle_peer_list(self, source, message: dftlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, dftlegacy_pb2.LegacyMessage.PL)
    ...
    def handle_sync(self, source, message: dftlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, dftlegacy_pb2.LegacyMessage.SYNC)
        if message.syncData.state == '':
            if source.factory.synced:
                source.send_sync(synced=True)

    @staticmethod
    def send_node_chain_state(dest_channel, node_chain_state: dft_pb2.NodeChainState):
        # FIXME: Not sure this belongs to peer management
        msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.CHAINSTATE,
    ...
    def broadcast_chain_state(self, node_chain_state: dft_pb2.NodeChainState):
        # FIXME: Not sure this belongs to peer management
    ...
    def handle_chain_state(self, source, message: dftlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, dftlegacy_pb2.LegacyMessage.CHAINSTATE)
    ...
    def handle_p2p_acknowledgement(self, source, message: dftlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, dftlegacy_pb2.LegacyMessage.P2P_ACK)
    ...
    def get_peers_stat(self) -> list:
        ...
        for source in list(self.peer_node_status.keys()):
            try:
                peer_stat = dft_pb2.PeerStat(peer_ip=source.peer.ip.encode(),
