...
from dft.core.messagereceipt import MessageReceipt
from dft.core.misc import ntp, logger
from dft.core.node import SyncState
from dft.core.p2p.p2pprotocol import P2PProtocol
from dft.core.p2p.IPMetadata import IPMetadata
from dft.core.processors.TxnProcessor import TxnProcessor
from dft.core.txs.MessageTransaction import MessageTransaction
from dft.core.txs.SlaveTransaction import SlaveTransaction
from dft.core.txs.LatticeTransaction import LatticeTransaction
from dft.core.txs.TokenTransaction import TokenTransaction
from dft.core.txs.TransferTokenTransaction import TransferTokenTransaction
from dft.core.txs.TransferTransaction import TransferTransaction
from dft.core.txs.multisig.MultiSigCreate import MultiSigCreate
from dft.core.txs.multisig.MultiSigSpend import MultiSigSpend
from dft.core.txs.multisig.MultiSigVote import MultiSigVote
from dft.generated import dftlegacy_pb2, dft_pb2

p2p_msg_priority = {
            dftlegacy_pb2.LegacyMessage.VE: 0,
            dftlegacy_pb2.LegacyMessage.PL: 0,
            dftlegacy_pb2.LegacyMessage.PONG: 0,

            ######################
            dftlegacy_pb2.LegacyMessage.MR: 2,
            dftlegacy_pb2.LegacyMessage.SFM: 1,

            dftlegacy_pb2.LegacyMessage.BK: 1,
            dftlegacy_pb2.LegacyMessage.FB: 0,
            dftlegacy_pb2.LegacyMessage.PB: 0,
            dftlegacy_pb2.LegacyMessage.BH: 1,

            ############################
            dftlegacy_pb2.LegacyMessage.TX: 1,
            dftlegacy_pb2.LegacyMessage.MT: 1,
            dftlegacy_pb2.LegacyMessage.TK: 1,
            dftlegacy_pb2.LegacyMessage.TT: 1,
            dftlegacy_pb2.LegacyMessage.LT: 1,
            dftlegacy_pb2.LegacyMessage.SL: 1,

            dftlegacy_pb2.LegacyMessage.EPH: 3,

            dftlegacy_pb2.LegacyMessage.SYNC: 0,
            dftlegacy_pb2.LegacyMessage.CHAINSTATE: 0,
            dftlegacy_pb2.LegacyMessage.HEADERHASHES: 1,
            dftlegacy_pb2.LegacyMessage.P2P_ACK: 0,

            dftlegacy_pb2.LegacyMessage.MC: 1,
            dftlegacy_pb2.LegacyMessage.MS: 1,
            dftlegacy_pb2.LegacyMessage.MV: 1,
        }
...
class P2PFactory(ServerFactory):
    ...
    def request_peer_blockheight(self):
        for peer in self._peer_connections:
            msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.BH,
                                              bhData=dft_pb2.BlockHeightData(block_number=0))
            peer.send(msg)
    ...
    def block_received(self, source, block: Block):
        ...
        if block.block_number != self._last_requested_block_number:
            ...
            self._dft_node.peer_manager.ban_channel(source)
            return
    ...
    def peer_fetch_block(self, retry=0):
        ...
        if retry >= 1:
            ...
            self._dft_node.peer_manager.ban_channel(self._target_channel)
    ...
    def compare_and_sync(self, source_peer, node_header_hash: dft_pb2.NodeHeaderHash):
    ...
    def request_full_message(self, mr_data: dftlegacy_pb2.MRData):
        """
        ...
        for peer in peers_list:
            ...
            msg = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.SFM,
                                              mrData=dftlegacy_pb2.MRData(hash=mr_data.hash, type=mr_data.type))
    ...
    def broadcast_tx(self, tx: TransferTransaction):
        ...
        if isinstance(tx, MessageTransaction):
            legacy_type = dftlegacy_pb2.LegacyMessage.MT
        elif isinstance(tx, TransferTransaction):
            legacy_type = dftlegacy_pb2.LegacyMessage.TX
        elif isinstance(tx, TokenTransaction):
            legacy_type = dftlegacy_pb2.LegacyMessage.TK
        elif isinstance(tx, TransferTokenTransaction):
            legacy_type = dftlegacy_pb2.LegacyMessage.TT
        elif isinstance(tx, SlaveTransaction):
            legacy_type = dftlegacy_pb2.LegacyMessage.SL
        elif isinstance(tx, LatticeTransaction):
            legacy_type = dftlegacy_pb2.LegacyMessage.LT
        elif isinstance(tx, MultiSigCreate):
            legacy_type = dftlegacy_pb2.LegacyMessage.MC
        elif isinstance(tx, MultiSigSpend):
            legacy_type = dftlegacy_pb2.LegacyMessage.MS
        elif isinstance(tx, MultiSigVote):
            legacy_type = dftlegacy_pb2.LegacyMessage.MV
        else:
    ...
    def broadcast_block(self, block: Block):
        ...
        self.register_and_broadcast(dftlegacy_pb2.LegacyMessage.BK, block.headerhash, block.pbdata, data)
    ...
    def broadcast(self, msg_type, msg_hash: bytes, mr_data=None):
        ...
        if not mr_data:
            mr_data = dftlegacy_pb2.MRData()

        mr_data.hash = msg_hash
        mr_data.type = msg_type
        data = dftlegacy_pb2.LegacyMessage(func_name=dftlegacy_pb2.LegacyMessage.MR,
                                           mrData=mr_data)
    ...
    def add_connection(self, conn_protocol) -> bool:
        ...
        if self._dft_node.peer_manager.is_banned(conn_protocol.peer):
            return False
        ...
        if conn_protocol.peer.ip == conn_protocol.host.ip and conn_protocol.peer.port == config.user.p2p_public_port:
            peer_list = [p for p in self._dft_node.peer_manager.known_peer_addresses if p != conn_protocol.peer.full_address]
            self._dft_node.peer_manager.extend_known_peers(peer_list)
            return False
    ...
    def monitor_connections(self):
        ...
        if len(self._peer_connections) == 0:
            ...
            known_peers = self._dft_node.peer_manager.load_known_peers()
