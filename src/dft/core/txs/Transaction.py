...
from pydftlib.pydftlib import bin2hstr, dftHelper, XmssFast
...
from dft.core.txs import build_tx as main_build_tx
...
class Transaction(object, metaclass=ABCMeta):
    ...
    def __init__(self, protobuf_transaction=None):
        ...
        if protobuf_transaction is None:
            self._data = dft_pb2.Transaction()
    ...
    def addr_from(self):
        ...
        return bytes(dftHelper.getAddress(self.PK))
    ...
    @staticmethod
    def from_pbdata(pbdata: dft_pb2.Transaction):
    ...
    @staticmethod
    def get_slave(tx):
        addr_from_pk = bytes(dftHelper.getAddress(tx.PK))
    ...
    def set_affected_address(self, addresses_set: set):
        ...
        addresses_set.add(bytes(dftHelper.getAddress(self.PK)))
    ...
    def validate_all(self, state_container: StateContainer, check_nonce=True) -> bool:
        ...
        addr_from_pk = bytes(dftHelper.getAddress(self.PK))
    ...
    def _coinbase_filter(self):
        if config.dev.coinbase_address in [bytes(dftHelper.getAddress(self.PK)), self.master_addr]:
