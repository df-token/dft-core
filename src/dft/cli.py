...
from pydftlib.pydftlib import mnemonic2bin, hstr2bin, bin2hstr

from dft.core import config
from dft.core.Wallet import Wallet, WalletDecryptionError
from dft.core.misc.helper import parse_hexblob, parse_qaddress
from dft.core.MultiSigAddressState import MultiSigAddressState
...
from dft.crypto.xmss import XMSS, hash_functions
from dft.generated import dft_pb2_grpc, dft_pb2

ENV_dft_WALLET_DIR = 'ENV_dft_WALLET_DIR'
...
class CLIContext(object):
    ...
    def get_stub_public_api(self):
        ...
        return dft_pb2_grpc.PublicAPIStub(channel)
...
def _public_get_address_balance(ctx, address):
    ...
    get_address_state_req = dft_pb2.GetAddressStateReq(address=parse_qaddress(address))
...
@click.version_option(version=config.dev.version, prog_name='dft Command Line Interface')
@click.group()
...
@click.option('--wallet_dir', default='.', help='local wallet dir', envvar=ENV_dft_WALLET_DIR)
@click.option('--json', default=False, is_flag=True, help='output in json')
@click.pass_context
def dft(ctx, verbose, host, port_pub, wallet_dir, json):
    """
    dft Command Line Interface
    """
...
def tx_push(ctx, txblob):
    ...
    stub = ctx.obj.get_stub_public_api()
    pushTransactionReq = dft_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@dft.command()
@click.option('--src', type=str, default='', prompt=True, help='signer dft address')
@click.option('--master', type=str, default='', prompt=True, help='master dft address')
@click.option('--addr_to', type=str, default='', prompt=True, help='dft Address receiving this message (optional)')
...
@dft.command()
@click.option('--src', type=str, default='', prompt=True, help='source dft address')
@click.option('--master', type=str, default='', prompt=True, help='master dft address')
...
@dft.command()
@click.option('--src', type=str, default='', prompt=True, help='signer dft address')
@click.option('--master', type=str, default='', help='master dft address')
...
@click.option('--owner', default='', prompt=True, help='Owner dft address')
...
def tx_token(ctx, src, master, symbol, name, owner, decimals, fee, ots_key_index):
    ...
    while True:
        ...
        initial_balances.append(dft_pb2.AddressAmount(address=parse_qaddress(address),
                                                      amount=amount))
...
def slave_tx_generate(ctx, src, master, number_of_slaves, access_type, fee, pk, ots_key_index):
    ...
    try:
        ...
        click.echo('Move slaves.json file from current directory to the mining node inside ~/.dft/')
    except Exception as e:
...
@dft.command()
@click.option('--owner', default='', prompt=True, help='source dft address')
@click.pass_context
def token_list(ctx, owner):
    ...
    try:
        stub = ctx.obj.get_stub_public_api()
        address_state_req = dft_pb2.GetAddressStateReq(address=owner_address)
        ...
        for token_hash in address_state_resp.state.tokens:
            get_object_req = dft_pb2.GetObjectReq(query=bytes(hstr2bin(token_hash)))
...
def state(ctx):
    ...
    """
    stub = ctx.obj.get_stub_public_api()
    nodeStateResp = stub.GetNodeState(dft_pb2.GetNodeStateReq())
